...
final class HomeViewController: BaseViewController {
    ...
    private lazy var showFavoritesButton: UIButton = {
        ...
        button.setTitle("Favorite Coins", for: .normal)
        button.setTitleColor(.white, for: .normal)
        button.titleLabel?.font = .systemFont(ofSize: 17, weight: .bold)
        button.backgroundColor = .systemBlue
        button.layer.cornerRadius = 10
        button.translatesAutoresizingMaskIntoConstraints = false
        button.addTarget(self, action: #selector(showFavoritesTapped), for: .touchUpInside)
        return button
    }()
    ...
    override func viewDidLoad() {
        ...
        Task {
            await viewModel.getFavoriteCoins(showLoader: false) { result in
                switch result {
    ...
    func refreshTable() {
        ...
        Task {
            await viewModel.getCoins( {  [weak self] result in
                switch result {
    ...
    @objc
    private func showFavoritesTapped() {
        viewModel.showFavoritesTapped()
    }
}
...
extension HomeViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.detailTextLabel?.text = "Price: \(coin.symbol) \(String(format: "%.4f", Double(coin.price)!))     24Hr Vol: \(coin._24hVolume) "
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let favorite = UIContextualAction(style: .normal, title: "Favorite") { action, view, complete in
            debugPrint("favorite")
            Task {
                await self.viewModel.favoriteTap(coin) { [weak self] result in
                    switch result {
                    case .success:
        ...
        let isFavorite = favoriteCoins.first(where: { $0.uuid == coin.uuid }) != nil
